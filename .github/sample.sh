...
cover_command+=" --lcov=\"$cur/coverage/lcov.info\" --output=\"$cur/coverage/\" --gitParserFile=\"$cur/coverage/gitparser.json\" --target-branch=\"$target_branch\" --source-branch=chore/pipeline-setup --report-format=console,json"
